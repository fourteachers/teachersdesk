...
function wp_pagenavi($before = '', $after = '') {
	...
	$blog_name_id = "SELECT post_id FROM $wpdb->postmeta WHERE meta_value like 'blog.php'";
	$blog_name_id = $wpdb->get_var($blog_name_id);
			...
			switch(intval($pagenavi_options['style'])) {
				case 1:
					...
					if ($start_page >= 2 && $pages_to_show < $max_page) {
						...
						$a_href = str_replace($site_url_link, $blog_page_link, clean_url(get_pagenum_link()));
						$a_href = str_replace($page->post_name.'/'.$page->post_name, $page->post_name, $a_href);
						echo '<li><a href="'.$a_href.'" class="first" title="'.$first_page_text.'">'.$first_page_text.'</a></li>';
					...
					for($i = $start_page; $i  <= $end_page; $i++) {						
						if($i == $paged) {
							...
							echo '
							<span class="current">'.$current_page_text.'</span>';
						} else {
							$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['page_text']);
							$a_href = str_replace($site_url_link, $blog_page_link, clean_url(get_pagenum_link($i)));
							$a_href = str_replace($page->post_name.'/'.$page->post_name, $page->post_name, $a_href);
							echo '<a href="'.$a_href.'" class="page" title="'.$page_text.'">'.$page_text.'</a>';
				...
				case 2;
					...
					for($i = 1; $i  <= $max_page; $i++) {
						...
						if($i == $paged) {
							$current_page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['current_text']);
							echo '<option value="'.str_replace($site_url_link, $blog_page_link, clean_url(get_pagenum_link($page_num))).'" selected="selected" class="current">'.$current_page_text."</option>\n";
...
function wp_pagenavi_archive($before = '', $after = '') {
